...
export class CostProvider {
    ...
    public async calculateCostsForFile(uri: vscode.Uri): Promise<CostEstimate | null> {
        try {
    ...
    public async getSuggestionsForFile(uri: vscode.Uri): Promise<any[]> {
        try {
            const workspaceFolder = vscode.workspace.getWorkspaceFolder(uri);
            if (!workspaceFolder) {
                return [];
    ...
    public async refreshAllCosts(): Promise<void> {
    ...
    public getCachedCosts(uri: vscode.Uri): CostEstimate | null {
        const workspaceFolder = vscode.workspace.getWorkspaceFolder(uri);
        if (!workspaceFolder) {
            return null;
        }
    ...
    private async executeTerraCostCommand(workspacePath: string, command: string, args: string[]): Promise<string | null> {
    ...
    private parseTerraCostOutput(output: string): CostEstimate {
    ...
    private parseSuggestionsOutput(output: string): any[] {
    ...
    private extractResourceType(resourceName: string): string {
